...
```{r Figure 3 and 4 - representative density plot, fig.height=6, fig.width=8}
...
srdb_comb %>% 
  filter(!is.na(Ecosystem) & !is.na(MAT)) %>% 
  filter(Ecosystem %in% c("Agriculture", "Desert", "Shrubland")) %>% 
  ggplot(aes(x = MAT, fill = factor(Source))) +
    facet_grid(rows = vars(Ecosystem),
               cols = vars(vs),
               scales = "free") +
...
# ggsave('outputs/Figure3.MAT.jpg', width = 8, height = 5)

# plot MAP density comparison
plot_grid(
  srdb_comb %>% 
  filter(!is.na(Ecosystem) & !is.na(MAP)) %>% 
  filter(Ecosystem %in% c("Agriculture")) %>% 
  filter(MAP <= 4000) %>% 
  ...
  # geom_violin(draw_quantiles = c(0.5))
  geom_density(alpha = 0.5) +
  labs(x=expression(""), y=expression(Density)),
  
  srdb_comb %>% 
  filter(!is.na(Ecosystem) & !is.na(MAP)) %>% 
  filter(Ecosystem %in% c("Desert", "Shrubland")) %>% 
  filter(MAP <= 1250) %>% 
  ggplot(aes(x = MAP, fill = factor(Source))) +
    facet_grid(rows = vars(Ecosystem),
               cols = vars(vs),
...
# ggsave('outputs/Figure5-Rs.jpg', width = 8, height = 6)
```
