...
## HC by month in south and north
```{r HC violin by partition method}
mycolors = c(brewer.pal(name="BrBG", n = 6), brewer.pal(name="PiYG", n = 6))
MGRhD %>% 
  select(Latitude, Rs_Norm, Rh_Norm, Ra_Norm, Meas_Month2) %>% 
  mutate(HC2 = round(Rh_Norm / Rs_Norm, 2),
         RC2 = round(Ra_Norm / Rs_Norm, 2)) %>%  
  mutate(Hetmosphere = case_when(
    Latitude > 0 ~ "North",
    Latitude <= 0 ~ "Sourth")
    ) %>% 
  # count(Partition_group)
  filter(HC2 > 0 & HC2 < 1) %>% 
  na.omit() %>% 
  ggplot(aes(x = Meas_Month2, y = HC2, fill = as.factor(Meas_Month2))) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) +
  # geom_jitter(shape = 16, position = position_jitter(0.2), col = "gray") +
  # geom_boxplot(width = 0.1) +
  theme(legend.title = element_blank()) +
  ...
  facet_grid(rows = vars(Hetmosphere)) + 
  xlab(expression(Measure~month)) +
  ylab(expression(Rh~":"~R[S])) +
